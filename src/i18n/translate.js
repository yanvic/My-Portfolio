import ProgressImage from "../assets/progress.jpeg"

export const translations = {
  en: {
    home: "Home",
    projects: "Projects",
    about: "About Me",
    contact: "Contact",
    blog: "Blog",
    nav: {
      home: "Home",
      projects: "Projects",
      about: "About",
      blog: "Blog"
    },
    welcome: "Welcome to my portfolio",
    description: "This is the main content...",
    hello: "Hi, I'm Yan",
    presentation: "Software developer focused on building clear, efficient, and well-structured digital solutions. I work across interfaces, backend systems, and automations, always prioritizing code quality, maintainability, and user experience. Strong collaborator, able to turn requirements into practical solutions and continuously improve products.",
    button_projects: "Projects Look",
    viewProject: "View project",
    viewResume: "View Resume",
    downloadResume: "Download",
    resume: "Resume",
    readBtn: "Read",
    viewAllArticles: "View all articles",
    careerTitle: "Career",
    kanbanTitle: "My personal Kanban",
    hobbiesTitle: "Beyond Code",
    hobbies: [
      { icon: "fa-camera", label: "Photography" },
      { icon: "fa-music", label: "Music" },
      { icon: "fa-futbol", label: "Football" },
      { icon: "fa-plane-departure", label: "Travel" },
      { icon: "fa-gamepad", label: "Gaming" },
    ],
    footer_title: "Ready for the Next Challenge",
    footer_desc: "Developer focused on quality, performance, and best practices, open to new professional challenges.",
    dev: "Developed by Yan Silva",
    blog_subtitle: "Articles about development, architecture and technology.",
    blog_sort_label: "Sort:",
    blog_sort_newest: "Newest",
    blog_sort_oldest: "Oldest",
    blog_clear_filter: "Clear filter",
    blog_back: "← Back",
    blog_read_article: "Read article →",
    blog_views_one: "read",
    blog_views_many: "reads",
    projects_card: [
      {
        title: "Trading Card Duel",
        desc: "Card game where the challenge is to determine which player has the higher Overall rating.",
        image: ProgressImage.src,
        link: "https://github.com/yanvic",
        github: "https://github.com/yanvic",
        techs: ["React", "TypeScript", "Node.js"]
      },
      {
        title: "FFMPEG",
        desc: "Project focused on demonstrating the power and practical use of FFMPEG across various applications.",
        image: ProgressImage.src,
        link: "https://github.com/yanvic",
        github: "https://github.com/yanvic",
        techs: ["Go", "Docker", "Linux"]
      },
      {
        title: "Excel and PDF",
        desc: "Tool for creating Excel and PDF layouts using macros, modern styling, and clean, professional design.",
        image: ProgressImage.src,
        link: "https://github.com/yanvic",
        github: "https://github.com/yanvic",
        techs: ["JavaScript", "Node.js", "AWS"]
      },
    ],
    articles_card: [
      {
        date: "15 JAN 24",
        tag: "Architecture",
        title: "Microservices in Practice",
        desc: "How to implement a microservices architecture that truly scales and works in production.",
        readTime: "8 min",
        views: "2.4k",
      },
      {
        date: "10 FEB 24",
        tag: "Development",
        title: "Effective Code Reviews",
        desc: "Best practices for conducting code reviews that enhance code quality and team collaboration.",
        readTime: "6 min",
        views: "1.8k",
      },
      {
        date: "05 MAR 24",
        tag: "DevOps",
        title: "CI/CD Pipelines Explained",
        desc: "A comprehensive guide to setting up continuous integration and continuous deployment pipelines.",
        readTime: "10 min",
        views: "3.1k",
      },
    ],
  },
  pt: {
    home: "Início",
    projects: "Projetos",
    about: "Sobre mim",
    contact: "Contato",
    blog: "Blog",
    nav: {
      home: "Início",
      projects: "Projetos",
      about: "Sobre",
      blog: "Blog"
    },
    welcome: "Bem-vindo ao meu portfólio",
    description: "Este é o conteúdo principal...",
    hello: "Oi, eu sou o Yan",
    presentation: "Desenvolvedor de software focado em criar soluções digitais claras, eficientes e bem estruturadas. Atuo no desenvolvimento de interfaces, sistemas backend e automações, sempre priorizando qualidade, organização do código e boa experiência do usuário. Tenho facilidade em colaborar com equipes, transformar requisitos em soluções práticas e evoluir produtos de forma contínua.",
    button_projects: "Ver Projetos",
    viewProject: "Ver projeto",
    viewResume: "Visualizar",
    downloadResume: "Baixar",
    resume: "Currículo",
    readBtn: "Ler",
    viewAllArticles: "Ver todos os artigos",
    careerTitle: "Carreira",
    kanbanTitle: "Meu Kanban pessoal",
    hobbiesTitle: "Além do Código",
    hobbies: [
      { icon: "fa-camera", label: "Fotografia" },
      { icon: "fa-music", label: "Música" },
      { icon: "fa-futbol", label: "Futebol" },
      { icon: "fa-plane-departure", label: "Viagens" },
      { icon: "fa-gamepad", label: "Games" },
    ],
    footer_title: "Pronto para o Próximo Desafio",
    footer_desc: "Desenvolvedor focado em qualidade, performance e boas práticas, aberto a novos desafios profissionais.",
    dev: "Desenvolvido por Yan Silva",
    blog_subtitle: "Artigos sobre desenvolvimento, arquitetura e tecnologia.",
    blog_sort_label: "Ordenar:",
    blog_sort_newest: "Mais recentes",
    blog_sort_oldest: "Mais antigos",
    blog_clear_filter: "Limpar filtro",
    blog_back: "← Voltar",
    blog_read_article: "Ler artigo →",
    blog_views_one: "leitura",
    blog_views_many: "leituras",
    projects_card: [
      {
        title: "Duelo de Cartinhas",
        desc: "Jogo de cartas onde o desafio é descobrir qual jogador tem o Overall maior.",
        image: ProgressImage.src,
        link: "https://github.com/yanvic",
        github: "https://github.com/yanvic",
        techs: ["React", "TypeScript", "Node.js"]
      },
      {
        title: "FFMPEG",
        desc: "Projeto dedicado para mostrar o poder e utilização do FFMPEG em diversas aplicações.",
        image: ProgressImage.src,
        link: "https://github.com/yanvic",
        github: "https://github.com/yanvic",
        techs: ["Go", "Docker", "Linux"]
      },
      {
        title: "Excel e PDF",
        desc: "Criador de layouts Excel e PDF com macros, estilização e layout moderno e bonito para diversas áreas.",
        image: ProgressImage.src,
        link: "https://github.com/yanvic",
        github: "https://github.com/yanvic",
        techs: ["JavaScript", "Node.js", "AWS"]
      },
    ],
    articles_card: [
      {
        date: "15 JAN 24",
        tag: "Arquitetura",
        title: "Microserviços na Prática",
        desc: "Como implementar uma arquitetura de microserviços que realmente escala e funciona em produção.",
        readTime: "8 min",
        views: "2.4k",
      },
      {
        date: "10 FEV 24",
        tag: "Desenvolvimento",
        title: "Revisões de Código Eficazes",
        desc: "Melhores práticas para conduzir revisões de código que aprimoram a qualidade do código e a colaboração da equipe.",
        readTime: "6 min",
        views: "1.8k",
      },
      {
        date: "05 MAR 24",
        tag: "DevOps",
        title: "Entendendo Pipelines CI/CD",
        desc: "Um guia completo para configurar pipelines de integração contínua e entrega contínua.",
        readTime: "10 min",
        views: "3.1k",
      }
    ],
  },
};